...
- **count**: Number of most recent videos to load (default: 300, max: 300 per notebook)
...
3. Fires `add_url` for each video with `--concurrency` parallel requests
4. Reports progress and saves errors to `/tmp/channel-load-errors.json`

**Performance:** ~300 videos in ~3-5 minutes at concurrency=6. Higher concurrency is faster but floods NotebookLM's confirmation step, producing false `FAIL` lines even when sources are actually added — keep it low and verify the real count in Step 5.
...
notebooklm source list --json | python3 -c "import json,sys; d=json.load(sys.stdin); print(f'{len(d)} sources loaded')"
```
...
# 4. Ask questions
notebooklm use 2a18a53d-...
notebooklm ask "What are the top product management frameworks discussed?"
```
