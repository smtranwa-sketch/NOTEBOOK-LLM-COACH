...
Usage:
  ...
  uv run --with "notebooklm-py[browser]" python3 load_channel.py load --videos /tmp/videos.json --notebook <id> --count 300 --concurrency 6
"""
import argparse
import asyncio
import json
...
def _extract_lockup(lvm: dict) -> dict | None:
    """Parse the newer lockupViewModel renderer (YouTube ~2025+)."""
    ...
    if not (vid_id and title):
        return None

    # length from thumbnail badge overlay
    length = ""
    try:
        overlays = lvm["contentImage"]["thumbnailViewModel"].get("overlays", [])
        for ov in overlays:
            for badge in ov.get("thumbnailBottomOverlayViewModel", {}).get("badges", []):
                txt = badge.get("thumbnailBadgeViewModel", {}).get("text", "")
                if txt and ":" in txt:
                    length = txt
    except (KeyError, TypeError):
        pass

    # views / published from metadata rows
    views, published = "", ""
    try:
...
def extract_videos(items: list) -> list[dict]:
    ...
    videos = []
    for item in items:
        # Newer lockupViewModel renderer (inside richItemRenderer or standalone)
        lvm = None
        if "richItemRenderer" in item:
            lvm = item["richItemRenderer"].get("content", {}).get("lockupViewModel")
        elif "lockupViewModel" in item:
            lvm = item["lockupViewModel"]
        if lvm:
            parsed = _extract_lockup(lvm)
            if parsed:
                videos.append(parsed)
            continue

        # Legacy videoRenderer / gridVideoRenderer
        vr = None
        if "richItemRenderer" in item:
...
NOTEBOOK_SOURCE_LIMIT = 300  # NotebookLM caps a single notebook at 300 sources
...
async def load_videos(videos_file: str, notebook_id: str, count: int, concurrency: int, skip: int):
    ...
    if count > NOTEBOOK_SOURCE_LIMIT:
        print(f"Note: NotebookLM caps a notebook at {NOTEBOOK_SOURCE_LIMIT} sources. "
              f"Requested {count}; loading the {NOTEBOOK_SOURCE_LIMIT} most recent instead.")
        count = NOTEBOOK_SOURCE_LIMIT
...
def main():
    ...
    lp.add_argument("--count", type=int, default=300, help="Number of videos to load (default: 300, NotebookLM's per-notebook cap)")
    lp.add_argument("--concurrency", type=int, default=6, help="Parallel requests (default: 6; higher values overwhelm NotebookLM's confirmation step and cause false 'failed' reports even though sources are added)")
